...
lazy_static! {
    static ref WORKERS_CACHE: Cache<String, Pool> = Cache::builder()
...
impl Service<Request<Body>> for RequestContext {
    ...
    fn call(&mut self, mut req: Request<Body>) -> Self::Future {
        ...
        let fut = async move {
            // get pool
            let pool = match WORKERS_CACHE.get(&wasm_path.to_str().unwrap().to_string()) {
                Some(pool) => pool,
                None => {
                    info!(
                    ...
                    WORKERS_CACHE.insert(wasm_path.to_str().unwrap().to_string(), pool.clone());
                    pool
                }
            };
